...
<head>
	...
	<script type="text/javascript" charset="UTF-8">
		...
		function del(code){
			...
			$.ajax({
			      type:"POST",
			      url: baseUrl + "/DelBoardController",
			      data: {code:codee, seq:seq},
			...
			location.href="${pageContext.request.contextPath }/DelBoardController?seq="+seq +"&board_code="+code;
			
			
		}
		
		function editboard(code){
			var codee = code;
			...
			$.ajax({
			      type:"POST",
			      url: baseUrl + "/EditBoardController",
			      data: {code:codee, title:title, content:content,seq:seq},
			      success: function(data){
			      },
			      error: function(e){
			      }
			...
			//		+"&board_title="+title+"&board_content="+content+"&board_code="+code;
					...
					<div class="row" >
						<div style="min-width:1000px;margin-left:100px;margin-top:50px"> 
							<div class="card shadow mb-4">
								...
								<div class="card-body">
								<c:if test="${board_code eq 'a'}">
									<c:set var="code" value="수도권"></c:set>
								</c:if>
								<c:if test="${board_code eq 'b'}">
									<c:set var="code" value="비수도권"></c:set>
								</c:if>
								<c:if test="${board_code eq 'c'}">
									<c:set var="code" value="문과"></c:set>
								</c:if>
								<c:if test="${board_code eq 'd'}">
									<c:set var="code" value="이과"></c:set>
								</c:if>
								<c:if test="${board_code eq 'e'}">
								...
								<c:if test="${b.board_code eq 'e'}">
									...
									<form id=jobpostform name=jobpostform action="${pageContext.request.contextPath }/WriteController?board_code=${board_code}" method=post>
									<div class="form-group">
										<div class="form-inline">
											<label for="code"><b>분류</b></label> 
											<input type=text class="form-control" id=code style="margin-left:35px; width:870px"name=code value="${code }" readonly>
										</div>
									</div>	
									<div class="form-group" style="margin-top:30px">
										<div class="form-inline">
											<label for="board_title"><b>제목</b></label> 
											<input type=text class="form-control" id=board_title name=board_title style="margin-left:35px; width:700px" placeholder="제목" value="${b.board_title}">
											
											<label for="postname" style="margin-left:20px;"><b>글번호</b></label> 
											<input type=text class="form-control" style="margin-left:20px; width:80px" id=boardseq name=boardseq value="${b.board_seq}" readonly>	
										</div>
									...
									<div class="form-group">
										<div class="form-inline">
											...
											<input type=text class="form-control" id=postcorpname style="margin-left:20px; width:870px" value="${nickname}" readonly>
										</div>
									</div>
									
									<div class="form-group">
										<div class="form-inline"> 
											<label for="postname"><b>조회수</b></label>
									...
									<div class="form-group">
										...
										<textarea style="width:940px" class="form-control" style="resize:none " placeholder="내용을 입력하세요" id=board_content name="board_content" rows=20>${b.board_content}</textarea>
									</div>
									<hr>
									<div class="form-group">
										<div class="form-inline">
											<input class="btn btn-primary btn-block" id=addpostbtn style="width:180px;" type=submit value=게시글추가>
											<input class="btn btn-primary btn-block" style="width:100px;margin-bottom:7px" type=button value=수정 onclick="editboard('${b.board_code}')">	
											<input class="btn btn-danger btn-block" style="width:100px;margin-bottom:7px" type=button value=삭제 onclick="del('${b.board_code}')">								
										</div>
									</div>
									</form>
